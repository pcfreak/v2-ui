...
uninstall_old_v2ray() {
    ...
    if [[ -f /usr/local/bin/v2ray ]]; then
        confirm "If v2ray installed in other ways is detected, whether to uninstall it, v2-ui comes with the official v2ray kernel, in order to prevent conflicts with its port, it is recommended to uninstall" "Y"
        if [[ $? != 0 ]]; then
            echo -e "${red}You have chosen not to uninstall, please ensure that v2ray and v2-ui installed by other scripts ${green}The official v2ray kernel that comes with it${red}No port conflict${plain}"
        else
            echo -e "${green}Start to uninstall v2ray installed in other ways${plain}"
            systemctl stop v2ray
...
install_v2-ui() {
    ...
    if  [ $# == 0 ] ;then
        ...
        echo -e "Latest version of v2-ui detected：${last_version}，start installation"
        ...
        echo -e "Start to install v2-ui v$1"
    ...
    echo -e "${green}v2-ui v${last_version}${plain} The installation is complete and the panel has started，"
    echo -e ""
    ...
    echo -e "If you want to modify 65432 to another port, enter the v2-ui command to modify, and also make sure that the port you modify is also allowed"
    echo -e ""
    ...
    echo -e "v2-ui start        - Launch v2-ui panel"
